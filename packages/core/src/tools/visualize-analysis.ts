...
const DEFAULT_OUTPUT_DIR = 'openrnd-analysis';
...
export interface VisualizeAnalysisParams {
   ...
   * workspace). Defaults to `<targetDir>/openrnd-analysis/<slug>-<ts>.html`.
   */
  output_path?: string;
 ...
 * mermaid UMD build, pointed to by OPENRND_MERMAID_JS) it is inlined so no
 * network is needed. Otherwise it loads the ESM build from a CDN (overridable
 * via OPENRND_MERMAID_URL). When neither is reachable, the diagram source still
...
<script>
  ...
  const url =
    process.env['OPENRND_MERMAID_URL'] ||
...
<div class="wrap">
  <header class="report">
    <h1>${safeTitle}</h1>
    <div class="meta">openrnd 소스코드 분석 · 생성: ${escapeHtml(generatedAt)}</div>
  </header>
  <main>
${bodyHtml}
  </main>
  <footer>Generated by openrnd · visualize_analysis</footer>
</div>
${mermaidLoader}
</body>
</html>
...
> {
  ...
  /** Resolves the output .html path, defaulting under <targetDir>/openrnd-analysis/. */
  ...
  async execute(_options: ExecuteOptions): Promise<ToolResult> {
    ...
    try {
      ...
      const localMermaidPath = process.env['OPENRND_MERMAID_JS'];
      if (localMermaidPath) {
        try {
          localMermaidJs = await fs.readFile(localMermaidPath, 'utf8');
        } catch (e) {
          debugLogger.warn(
            `[visualize_analysis] Could not read OPENRND_MERMAID_JS (${localMermaidPath}): ${getErrorMessage(e)}. Falling back to CDN.`,
          );
        }
      }
  ...
  constructor(
    ...
    super(
      ...
      'Create a visual HTML report from a source-code / business-logic analysis and open it in ' +
        ...
        'self-contained HTML file, saves it (default under <workspace>/openrnd-analysis/), and ' +
        ...
        properties: {
          ...
          output_path: {
            ...
            description:
              ...
              'Defaults to <workspace>/openrnd-analysis/<slug>-<timestamp>.html.',
          },
          open: {
            type: 'boolean',
            description:
