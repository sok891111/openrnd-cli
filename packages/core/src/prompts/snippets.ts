...
- **Creating PowerPoint decks (HTML-first):** When the user asks you to MAKE slides / a deck / a report deck (e.g. \`PPT 만들어줘\`, \`슬라이드/장표로 정리해줘\`, \`발표자료 만들어줘\`), use the \`create_pptx\` tool. It builds the deck by RENDERING AN HTML SLIDE DECK to images with headless Chrome and packing them into a \`.pptx\` (one full-bleed image per slide) — no PowerPoint needed; works on macOS/Windows/Linux. WORKFLOW: (1) author ONE self-contained HTML document where each slide is a top-level \`<section class="slide">\` element, every \`.slide\` the SAME fixed size as the deck aspect (16:9 ⇒ 1280×720 px). Design it richly with inline CSS — action-title typography (a full-sentence takeaway per slide, not a topic label), KPI/stat cards, comparison columns, process/timeline flows, icons via inline \`<svg>\`, charts (inline \`<svg>\` or a CDN chart lib), and brand color; one idea per slide and nothing taller than a slide (overflow is clipped). For Korean (or other CJK) text, set \`word-break: keep-all\` so lines wrap at word boundaries rather than mid-word (the tool applies this by default too). (2) Call \`create_pptx\` with that HTML as \`html\` (or save the file and pass \`html_path\`); optional \`aspect\` (\`16:9\` default / \`4:3\`) and \`output_path\`. The result preserves your visual design EXACTLY, but the slides are IMAGES — they are NOT text-editable in PowerPoint; the tool ALSO saves the original \`.html\` next to the \`.pptx\` and reports BOTH paths, so the user can re-edit the HTML and regenerate — surface both paths to the user. To match an existing in-house deck's look, read it for its colors/fonts/structure and reproduce that styling in your HTML. Do NOT hand-write \`.pptx\` XML or generate decks with \`run_python\`/\`python-pptx\`; always go through \`create_pptx\`.
