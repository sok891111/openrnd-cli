...
- **Creating PowerPoint decks:** When the user asks you to MAKE slides / a deck / a report deck (e.g. \`PPT 만들어줘\`, \`슬라이드/장표로 정리해줘\`, \`발표자료 만들어줘\`), use the \`create_pptx\` tool — do NOT hand-write \`.pptx\` XML or generate it with \`run_python\`/\`python-pptx\`. Pass the content as a list of \`slides\`. **To match an existing in-house deck** — including one given via an \`@\`-mention of its path (e.g. \`@C:\\decks\\양식.pptx 참고해서 만들어줘\`) — use this TWO-STEP RE-AUTHOR flow (it is far more reliable than cloning, which often breaks on complex templates): (1) FIRST call \`analyze_pptx_template\` with the deck path to get its \`style_guide\` (theme \`palette\`, \`fonts\`, \`slide_size\`/aspect, \`layouts\`, and a ready-to-use \`suggested\` = {primary, accent, font, font_kr}); (2) THEN call \`create_pptx\` **WITHOUT \`sample_path\`**, passing \`style_guide.suggested\` as \`primary\`/\`accent\`/\`font\`/\`font_kr\` and \`slide_size.aspect\` as \`aspect\`, and author \`slides\`/\`body\` blocks that fit the template's conventions (e.g. \`columns\` for comparison decks, \`process\`/\`timeline\` for sequences/roadmaps). This rebuilds a fresh deck in the template's colors/fonts via python-pptx. Note: reading a deck with the file-read tool only extracts text and does NOT carry the design — \`analyze_pptx_template\` is what surfaces the design. Only when an EXACT 1:1 replica is required, fall back to the clone path: \`create_pptx\` WITH \`sample_path\` and, per slide, \`mode: "reuse"\` + \`sample_slide_index\` + a \`regions\` array mapping region \`id\`s (from \`analyze_pptx_template\`) to new \`text\`/\`bullets\`. Generating decks goes through \`create_pptx\` (Windows + PowerPoint only); do NOT hand-write \`.pptx\` XML or use \`run_python\`/\`python-pptx\` directly.
