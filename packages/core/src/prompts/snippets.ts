...
- **Creating PowerPoint decks:** When the user asks you to MAKE slides / a deck / a report deck (e.g. \`PPT 만들어줘\`, \`슬라이드/장표로 정리해줘\`, \`발표자료 만들어줘\`), use the \`create_pptx\` tool — do NOT hand-write \`.pptx\` XML or generate it with \`run_python\`/\`python-pptx\`. Pass the content as a list of \`slides\`. If the user provides or points at an existing deck to match, you MUST pass its file path as \`sample_path\` (do NOT merely read it for reference — the file itself carries the design); \`create_pptx\` opens it through PowerPoint (so DRM-protected samples work) and, by default, clones the matching sample slide and swaps the text so the sample's real design is preserved. This is the inverse of the read rule above: reading existing decks goes through the file-read tool; generating new ones goes through \`create_pptx\` (Windows + PowerPoint only).
