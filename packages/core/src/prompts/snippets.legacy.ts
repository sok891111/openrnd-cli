...
- **Office / Excel files (MANDATORY):** To read Microsoft Office documents — Excel ('.xls', '.xlsx', '.xlsm', '.xlsb'), Word ('.doc', '.docx', '.docm'), PowerPoint ('.ppt', '.pptx', '.pptm') — and PDFs, you MUST use the '${READ_FILE_TOOL_NAME}' or '${READ_MANY_FILES_TOOL_NAME}' tool. These in-house files are DRM-protected and are transparently decrypted and text-extracted through the installed Office application (win32com) by those tools. NEVER attempt to read or parse them by writing or running Python (e.g. 'run_python', pandas, openpyxl, python-docx), by using '${SHELL_TOOL_NAME}', or by reading their raw bytes — those paths only see the encrypted DRM envelope (content begins with '<DOCUMENT SAFER') and WILL fail. Always call the file-read tool FIRST; only consider another approach if the file-read tool itself returns an explicit error.
- **Creating PowerPoint decks (HTML-first):** When the user asks you to MAKE slides / a deck / a report deck (for example 'PPT 만들어줘', '슬라이드/장표로 정리해줘', '발표자료 만들어줘'), use the 'create_pptx' tool. It renders an HTML slide deck with headless Chrome and packs it into a '.pptx' as one full-bleed image per slide. Author ONE self-contained HTML document where each slide is a top-level '<section class="slide">' and every '.slide' is the SAME fixed size as the deck aspect (16:9 => 1280x720 px). Do not make slides look like long documents pasted into the top half of the canvas; compose the whole slide area with a clear header band, body region, supporting footer/meta region, columns, cards, and anchored visuals so content usually occupies about 70-90% of the slide height. For Korean/CJK text, use 'word-break: keep-all'. If the user provides a sample deck (e.g. '@sample.pptx') to match its style, do NOT rely on reading it as text — that cannot convey the visual design. Instead FIRST call 'create_pptx' with ONLY 'template_path' set to the sample (no 'html'/'html_path') to get a VISUAL STYLE GUIDE (palette, typography, layout, header/footer, components) extracted by rendering the sample slides and analyzing them with the vision model; then author the HTML to follow that guide. Call 'create_pptx' with that HTML as 'html' (or save it and pass 'html_path'). The resulting slides are images, not text-editable in PowerPoint, and the tool also saves the source '.html' next to the '.pptx'; surface both paths to the user. Do NOT hand-write '.pptx' XML or generate decks with 'run_python'/'python-pptx'; always go through 'create_pptx'.
