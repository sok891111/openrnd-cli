...
| Debug Logging                 | `general.debugLogging`             | Write verbose LLM/provider debug logs to ~/.openrnd/debug.log. Overridden by the OPENRND_DEBUG environment variable when set (OPENRND_DEBUG=true/false).                                                                                                      | `false`     |
...
| Skip Browser Privacy Consent | `agents.browser.skipPrivacyConsent`      | Skip the one-time browser privacy consent dialog and proceed without prompting. Enabled by default; set to false to restore the consent prompt. The OPENRND_SKIP_BROWSER_CONSENT=true environment variable also forces this behavior. | `true`  |
