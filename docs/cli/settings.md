...
| Debug Logging                 | `general.debugLogging`             | Write verbose LLM/provider debug logs to ~/.openwork/debug.log. Overridden by the OPENWORK_DEBUG environment variable when set (OPENWORK_DEBUG=true/false).                                                                                                   | `false`     |
...
| Skip Browser Privacy Consent | `agents.browser.skipPrivacyConsent`      | Skip the one-time browser privacy consent dialog and proceed without prompting. Enabled by default; set to false to restore the consent prompt. The OPENWORK_SKIP_BROWSER_CONSENT=true environment variable also forces this behavior. | `true`  |
